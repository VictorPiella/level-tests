...
const getDCHeroes = (heroes) => {
  return (heroes.filter((x)=>x.publisher === "DC Comics"));
}
...
const groupByPublisher = (heroes) => {
  return heroes.reduce((previousValue, heroe) => {
    return {
      ...previousValue,
      [heroe.publisher]: [
        ...(previousValue[heroe.publisher] || []),
        heroe,
      ],
    }
  }, {})
...
const getDCHeroesWithMoreThanOneCharacter = (heroes) => {
  return convertCharactersToArray(heroes).filter((heroe) => heroe.characters.length > 1 );
}
